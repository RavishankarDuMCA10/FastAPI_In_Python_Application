...
engine = create_engine(
    'mysql+mysqlconnector://root:ravishankar2247@localhost:3306/books',
    echo=True
)
...
def add_book(book:Book, author:Author):
    with Session(engine) as session:
        exiting_book = session.execute(select(Book).filter(Book.title == book.title, Book.number_of_pages == book.number_of_pages)).scalar()
        if exiting_book is not None:
        ...
        if existing_author is not None:
            ...
            session.flush()
            paring = BookAuthor(author_id=existing_author.author_id, book_id=book.book_id)
        else:
            print("Author does not exist! Adding author")
            session.add(author)
            session.flush()
            paring = BookAuthor(author_id=author.author_id, book_id=book.book_id)
        session.add(paring)
        session.commit()
        print("New pairing added " + str(paring))
